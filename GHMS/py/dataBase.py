...
class dataBase:
	...
	def check(self, username, password, type):
		if type=='admin':
			admin=get_object_or_404(Admin, Name=username)
	...
	def getBookings(self, type, GNR=0, usr=None, rum=None):
		if(type=='admin'):
			...
			bukings=get_object_or_404(Booking, type='reserved')
			return bukings
		else:
			if(usr!=None):
				bukings=get_object_or_404(Booking, user_id=usr.id)
			...
			bukings=get_object_or_404(Booking, type='unreserved')
			return bukings
	...
	def deleteObj(seelf, obj):
		if(isinstance(obj, User)):
			...
			Admin.objects.filter(id=obj.id).delete()
		elif(isinstance(obj, Room)):
